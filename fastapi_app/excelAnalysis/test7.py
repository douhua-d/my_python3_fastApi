...
# 按医生分组统计6月份总卡号数量、新增卡号数量和药品数量
doctor_stats = []
for doctor, group in june_data.groupby('医生'):
    # 获取该医生的总卡号数
...
doctor_stats_df = pd.DataFrame(doctor_stats).sort_values('6月新增卡号数量', ascending=False)

# 药品统计分析
medicine_stats = []

# 全部药品统计
if '药品' in june_data.columns and '数量' in june_data.columns:
    # 按药品分组统计
    for medicine, group in june_data.groupby('药品'):
        # 总数量
...
with pd.ExcelWriter('6月卡号药品分析报告.xlsx', engine='openpyxl') as writer:
    # 1. 医生统计表（现在包含药品和单位信息）
    ...
    # 4. 全部6月数据(可筛选)
    june_data.to_excel(writer, sheet_name='6月全部数据', index=False)
    
    # 5. 全部新增卡号数据(可筛选)
    new_cards_data.to_excel(writer, sheet_name='6月新增卡号', index=False)
    ...
    # 7. 主要药品分析（取前10种药品）
    if '药品' in june_data.columns:
        top_medicines = june_data['药品'].value_counts().head(10)
...
# 添加自动筛选功能
wb = load_workbook('6月卡号药品分析报告.xlsx')
for sheet_name in ['医生统计', '药品统计', '月度对比', '6月全部数据', '6月新增卡号', '主要药品TOP10']:
    if sheet_name in wb.sheetnames:
        ws = wb[sheet_name]
        ws.auto_filter.ref = ws.dimensions

# 保存工作簿
wb.save('6月卡号药品分析报告.xlsx')

print('6月卡号药品分析报告已导出为 6月卡号药品分析报告.xlsx')
print(f"6月总卡号数: {june_data['就诊卡号'].nunique()}")
print(f"6月新增卡号数: {len(new_cards_data)}")
print(f"6月新增占比: {(len(new_cards_data) / june_data['就诊卡号'].nunique() * 100):.2f}%") 