...
# Excel 文件路径（与脚本同层级）
files = {
    'November': '可力洛11月.xlsx',
    'October': '可力洛10月.xlsx',
    'September': '可力洛9月.xlsx',
    'August': '可力洛8月.xlsx',
    'July': '可力洛7月.xlsx',
    'June': '可力洛6月.xlsx',
    'May': '可力洛5月.xlsx',
...
# 获取11月数据
if 'November' not in dataframes or dataframes['November'].empty:
    print("11月数据文件不存在或为空，请检查文件路径")
    exit()

november_data = dataframes['November']

# 将11月数据中就诊卡号不在之前月份的行筛选出来，即新增卡号
new_cards_data = november_data[~november_data['就诊卡号'].astype(str).isin(previous_cards)]

# 按医生分组统计11月份总卡号数量、新增卡号数量和药品数量
doctor_stats = []
for doctor, group in november_data.groupby('医生'):
    # 获取该医生的总卡号数
...
doctor_stats_df = pd.DataFrame(doctor_stats).sort_values('11月新增卡号数量', ascending=False)

# 药品统计分析
medicine_stats = []

# 全部药品统计
if '药品' in november_data.columns and '数量' in november_data.columns:
    # 按药品分组统计
    for medicine, group in november_data.groupby('药品'):
        # 总数量
...
with pd.ExcelWriter('11月卡号药品分析报告.xlsx', engine='openpyxl') as writer:
    # 1. 医生统计表（现在包含药品和单位信息）
    ...
    # 4. 全部11月数据(可筛选)
    november_data.to_excel(writer, sheet_name='11月全部数据', index=False)
    
    # 5. 全部新增卡号数据(可筛选)
    new_cards_data.to_excel(writer, sheet_name='11月新增卡号', index=False)
    ...
    # 7. 主要药品分析（取前10种药品）
    if '药品' in november_data.columns:
        top_medicines = november_data['药品'].value_counts().head(10)
...
# 添加自动筛选功能
wb = load_workbook('11月卡号药品分析报告.xlsx')
for sheet_name in ['医生统计', '药品统计', '月度对比', '11月全部数据', '11月新增卡号', '主要药品TOP10']:
    if sheet_name in wb.sheetnames:
        ws = wb[sheet_name]
        ws.auto_filter.ref = ws.dimensions

# 保存工作簿
wb.save('11月卡号药品分析报告.xlsx')

print('11月卡号药品分析报告已导出为 11月卡号药品分析报告.xlsx')
print(f"11月总卡号数: {november_data['就诊卡号'].nunique()}")
print(f"11月新增卡号数: {len(new_cards_data)}")
print(f"11月新增占比: {(len(new_cards_data) / november_data['就诊卡号'].nunique() * 100):.2f}%")
